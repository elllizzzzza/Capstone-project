...
public class BookService {
    ...
    public PagedResponse<BookDTO> getAllBooks(BookFilter filter, Pageable pageable) {
        return PagedResponse.from(
                bookRepository.findAll(BookSpecification.fromFilter(filter), pageable)
                        .map(b -> bookMapper.mapToDTO(b, new BookDTO()))
        );
    }

    public BookDTO getBookById(Long id) {
        return bookMapper.mapToDTO(findOrThrow(id), new BookDTO());
    }

    public List<BookDTO> searchBooks(String keyword) {
        return bookRepository.findByTitleContainingIgnoreCase(keyword).stream()
                .map(b -> bookMapper.mapToDTO((Book) b, new BookDTO()))
                .toList();
    }
    ...
    public BookDTO addBook(BookDTO dto) {
        Book book = bookMapper.mapToEntity(dto, new Book());
        bookRepository.save(book);
        return bookMapper.mapToDTO(book, new BookDTO());
    }

    @Transactional
    public BookDTO updateBook(Long id, BookDTO dto) {
        Book book = findOrThrow(id);
        bookMapper.mapToEntity(dto, book);
        bookRepository.save(book);
        return bookMapper.mapToDTO(book, new BookDTO());
    }
